...
# Plot
#png('/Users/rmadhok/Dropbox (Personal)/def_biodiv/docs/tex_doc/fig/tree_cover.png')
...
```{r,echo=FALSE,warning=FALSE,message=FALSE,include=FALSE}
# Read Data
tree_cover <- st_read(paste('def_biodiv/data/csv/', sep=''), 
                         'forest_cover', stringsAsFactors = F)
  
tree_cover <- tree_cover %>%
  mutate(tree_cover_mean = as.numeric(tree_cover_mean)) %>%
  filter(year == 2018) %>% 
  dplyr::select(c_code_2011, tree_cover_mean) %>%
  rename(c_code_11 = c_code_2011)
  
india_dist_def <- left_join(india_districts, tree_cover, by="c_code_11")
```


```{r,echo=FALSE,warning=FALSE,message=FALSE,fig.height=6,fig.width=8}
# Plot Tree Cover
tree_plot <- ggplot(india_dist_def, aes(fill = tree_cover_mean)) +
...
#        legend.text=element_text(size=15))
#tree_plot
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/tree_cover_plot.png', sep=''))
```
...
defn <- st_read(paste('def_biodiv/data/csv/', sep=''), 
                         'fc_dist_ym_main', stringsAsFactors = F)

# Prepare 2015 and 2018 cross-section
defn_year <- defn %>% 
  mutate(dist_f_cum_km2 = as.numeric(dist_f_cum_km2)) %>%
  arrange(c_code_2011, year_month) %>%
  group_by(c_code_2011, year) %>%
  summarize(dist_f_cum_km2 = last(dist_f_cum_km2)) %>%
...
ggplot(india_dist_def, aes(fill = dist_f_cum_km2)) +
 geom_sf(alpha = 0.8, colour = 'white', size = 0.3) +
...
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/deforestation.png', sep=''))
```
...
defn_change <- spread(defn_year, year, dist_f_cum_km2)
defn_change$theta_dist_f_cum_km2 <- defn_change$`2018`-defn_change$`2015`
...
def_plot <- ggplot(india_dist_def, aes(fill = theta_dist_f_cum_km2)) +
  geom_sf(alpha = 0.8, colour = 'white', size = 0.3) +
  scale_fill_viridis(name = '',
                     direction = -1,
                     guide = guide_colourbar(
  ...
  theme(line = element_blank(),
        ...
        legend.text=element_text(size=15))
def_plot
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/theta_deforestation.png', sep=''))
```
...
ebird <- st_read(paste('def_biodiv/data/csv', sep=''),
                   'ebird_user', stringsAsFactors = F)

# Truncate at 99th percentile
ebird_trunc <- ebird %>%
  mutate(n_trips = as.numeric(n_trips)) %>%
  filter(n_trips < quantile(n_trips, 0.99))

# Summarize trips per district
trips <- ebird_trunc %>%
  group_by(c_code_2011) %>%
  summarize(n_trips = mean(n_trips, na.rm=T)) %>%
  rename(c_code_11 = c_code_2011)
trips <- trips %>% mutate(z_trips = (n_trips-mean(n_trips))/sd(n_trips))

india_dist_def <- left_join(india_dist_def, trips, by="c_code_11")
#india_dist_def$sd_trips[india_dist_def$theta_dist_f_cum_km2==0] <- NA
```

```{r,echo=FALSE,warning=FALSE,message=FALSE,include=FALSE}
india_dist_def$group[india_dist_def$theta_dist_f_cum_km2==0] <- 'control'
india_dist_def$group[india_dist_def$theta_dist_f_cum_km2>0 | is.na(india_dist_def$theta_dist_f_cum_km2) ] <- 'treatment'

# All Districts
trip_plot <- ggplot(india_dist_def, aes(fill = z_trips)) +
  geom_sf(alpha = 0.8, colour = 'white', size = 0.3) +
  ...
  theme(line = element_blank(),
        ...
        legend.title = element_text(size=10),
        legend.text=element_text(size=15))
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/n_trips_district.png', sep=''))

# Highlight Treatment
trip_plot_hl <- ggplot() +
  geom_sf(data=india_dist_def[india_dist_def$group=='control',], 
          aes(fill=z_trips), alpha = 0.8, colour = 'white', size = 0.3) +
  geom_sf(data=india_dist_def[india_dist_def$group=='treatment',], 
          aes(fill=z_trips), alpha = 0.8, colour = 'black', size = 0.3) +
  scale_fill_viridis(option='plasma',
                     name = 'Number of Trips (std. dev.)',
                     direction = -1,
                     guide = guide_colourbar(
                       direction = "horizontal",
                       barheight = unit(5, units = "mm"),
  ...
  theme(line = element_blank(),
        ...
        legend.text = element_text(size = 15))
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/n_trips_district_hl.png', sep=''))
```
...
```{r,echo=FALSE,warning=FALSE,message=FALSE,include=FALSE}
# Distribution of birding activity

# Tag treatment/control
defn_trips <- defn_year
defn_trips$group[defn_trips$dist_f_cum_km2>0] <- 'Treatment'
defn_trips$group[defn_trips$dist_f_cum_km2==0] <- 'Control'

# Merge Trips
ebird_yr <- ebird %>%
  mutate(n_trips = as.numeric(n_trips)) %>%
  filter(n_trips < quantile(n_trips, 0.90)) %>%
  group_by(c_code_2011, year) %>%
  summarize(n_trips = sum(n_trips, na.rm=T)) %>%
  filter(year == 2015 | year == 2018) %>%
  rename(c_code_11 = c_code_2011)
defn_trips <- merge(defn_trips, ebird_yr, by=c('c_code_11', 'year'))

# Plot
ggplot(defn_trips, aes(n_trips, fill = group)) + 
   geom_histogram(alpha = 0.5, aes(y = ..density..), position = 'identity') +
  facet_wrap(~year, scales='free')
```

## Spatial Coverage of Birding Activity in Deforestation Districts
The map in panel B shows the spatial coverage of bird-watching activity. This is measured by gridding the district map into 5km x 5km cells and computing the percentage of cells with at least one bird sighting. This tells us whether bird-watching is concentrated within a district or not. It is useful for capturing access, since infrastructure projects may open up access to pristine parts of the forest. 
...
coverage <- st_read(paste('def_biodiv/data/csv/', sep=''), 
                'coverage_dist_grid_5km', stringsAsFactors = F)
coverage <- coverage %>%
...
```{r,echo=FALSE,warning=FALSE,message=FALSE,fig.height=6,fig.width=8}
# Combine spatial coverage and deforestation
...
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/cdf_n_trips.png', sep=''))
```
...
```{r,echo=FALSE,warning=FALSE,message=FALSE,fig.height=6,fig.width=8}
n_trips_month_plot <- ggplot(ebird_trips_month, aes(x=month, y=(n_trips/1000))) +
  geom_bar(stat="identity", width=0.5) +
  ylab('Number of Trips (Thousands)\n') +
...
n_trips_month_plot
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/n_trips_month.png', sep=''))
```
...
sp_ym
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/n_species_ym.png', sep=''))
```
...
sp_yr <- ggplot(n_species_yr, aes(x=YEAR, y=n_species)) +
  ...
  theme(panel.grid.minor = element_blank(), 
        ...
        axis.title.x = element_blank(),
        axis.ticks = element_blank())
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/n_species_yr.png', sep=''))
```
...
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/n_species_yr_ym.png', sep=''))
```

```{r,echo=FALSE,warning=FALSE,message=FALSE,include=FALSE}
# Read
n_species_uyr <- st_read(paste('def_biodiv/data/csv/', sep=''), 
                    'n_species_uyr', stringsAsFactors = F)

# Number Species per User-Year
n_species_uyr <- n_species_uyr %>%
  mutate(n_species=as.numeric(n_species)) %>%
  filter(YEAR!="2019")
```

```{r,echo=FALSE,warning=FALSE,message=FALSE,include=FALSE}
sr_uyr <- n_species_uyr %>%
  mutate(YEAR=as.numeric(YEAR)) %>%
  group_by(YEAR) %>%
  summarize(md_sr = median(n_species, na.rm=T),
            mn_sr = mean(n_species, na.rm=T))
  
sr_uyr_plot <- ggplot(sr_uyr, aes(x=YEAR, y=mn_sr)) +
  geom_line() +
  geom_point() +
  ...
  theme(panel.grid.minor = element_blank(), 
        ...
        axis.ticks = element_blank(),
        text = element_text(size=17))
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/mean_species_uyr.png', sep=''))
```
...
```{r,echo=FALSE,warning=FALSE,message=FALSE,fig.height=6,fig.width=9}
# Combine year and year-month species plots
ggarrange(sp_ym, n_trips_month_plot, nrow=2, align='h')
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/seasonality.png', sep=''))
```

# User Characteristics
The user-level data includes individual identifiers, so I can track an individual's trips across time and space. The plots below show the distribution of *per-user* characteristics. Across the sample period, most users take around 20 trips across 15 time periods and travel through 6 states and 10 districts. This provides a huge amount of variation to exploit in the trip-level analysis with individual fixed effects. 

```{r,echo=FALSE,warning=FALSE,message=FALSE,include=FALSE}
ebird$state_code_2011 <- substr(ebird$c_code_2011,1,3)

# User-level Characteristics
ebird_user <- ebird %>%
  group_by(OBSERVER.ID) %>%
  summarize("No. of Trips" = sum(as.numeric(n_trips)),
...
ggsave(paste('def_biodiv/docs/jmp/tex_doc/fig/user_charac_hist.png', sep=''))
```