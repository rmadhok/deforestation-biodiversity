...
SAVE.PATH <- '/Volumes/FreeAgent GoFlex Drive/Data/def_biodiv/'
...
### Add eBird Sightings
proportion <- 5
ebird <- read.csv(paste('data/csv/ebird_sample_', proportion, '.csv', sep=''), stringsAsFactors=F)
...
for(i in 1:dim(ebird)[1]){
  ...
  for(j in J){
    ...
    print(paste('Inserting row: ', i))
    write.table(row, 
                paste(SAVE.PATH, 'villages_ebird_xwalk_', proportion, '.txt', sep=''), 
                append=TRUE,sep="\t", 
                row.names=FALSE)
    #villages.ebird.xwalk <- rbind(villages.ebird.xwalk,row)
  }
