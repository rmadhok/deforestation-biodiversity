...
if `project' == 1 {
	...
	* Merge seats (496 districts 2014-2018)
	merge m:1 c_code_2011 year_month using "${DATA}/dta/polecon_v02", keep(1 3) keepusing(*_seats *_state_share) nogen
	...
	* Mechanisms
	...
	esttab using "${TABLE}/v3/tables/mech_polecon.tex", replace ///
		keep(st_seats) stats(ymean st_y_fe clust N r2, labels(`"Outcome Mean"' ///
		`"State $\times$ Year FEs"' `"Clustering"' `"N"' `"\(R^{2}\)"') ///
		fmt(3 0 0 0 3)) mlabels("CBA (=1)" "FRA (=1)" "Displ. (=1)" "Displ. (Fam.)" ///
