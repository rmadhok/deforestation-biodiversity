...
if `hte' == 1 {
	...
	* Aggregate to 1991 border
	collapse (mean) temp rain duration distance exp_idx coverage_udym ///
					traveling group_size rad_sum sr tree_cover_s hour  ///
	...
	* HTE
	...
	eststo: reghdfe sr c.dist_f_cum_km2##c.(mahrai st_share tree_cover_base_s n_*_base) `ctrls', ///
			a(uid#year c_code_1991_num state_code_1991_num#month) vce(cl biome)
	...
	* Table
	esttab using "${TABLE}/tables/hte_polecon.tex", replace ///
		...
		indicate("Infrastructure $\times$ ST Share=c.*#c.st_share" ///
				 "Infrastructure $\times$ Forest Cover=c.*#c.tree_cover_base_s" ///
				 "Infrastructure $\times$ Category Shares=c.*#c.n_*_base") ///
		nocons nonotes booktabs nomtitles ///
...
if `project' == 1 {
	...
	* Get controls
	merge m:1 c_code_2011 year_month using "${DATA}/dta/fc_dym_s2_v02", keepus(tree_cover_base) keep(1 3) nogen
	merge m:1 c_code_2011 using "${DATA}/dta/2011_india_dist", keepus(tot_area tot_pop) keep(3) nogen
	g encroach_frac = dist_f / (dist_f + dist_nf)
	g tree_cover_base_s = (tree_cover_base / tot_area)*100
	...
	* Mechanisms
	local controls st_share tree_cover_base_s tot_area dist_f lat
	foreach v of varlist proj_fra_num cba_num  proj_in_pa_esz {
	...
	* Keywords
	...
	g tribal = regexm(notes, "tribals") | regexm(notes, "tribe") | regexm(notes, "scheduled tribe") | regexm(notes, "ST") | regexm(notes, "forest dwel")
	g forest = regexm(notes, "forest") | regexm(notes, "forestland") | regexm(notes, "forest diversion")
	g project = regexm(notes, "project")
	g forest_project = forest | project
	
	* district-level
	merge m:1 c_code_2011 using "${DATA}/dta/crosswalk_full", keepusing(c_code_1991) keep(3) nogen
	
	collapse (sum) n_protest = forest_project, by(c_code_2011 year)
	tempfile protests
	save "`protests'"
	...
	* Merge
	...
	merge m:1 c_code_2011 using "${DATA}/dta/crosswalk_full", keepusing(c_code_1991) keep(3) nogen
	merge 1:m c_code_2011 year using "`temp'", keep(2 3)
	g log_n_protest = asinh(n_protest)
	
	eststo: reghdfe log_n_protest mahrai `controls' tot_pop, ///
