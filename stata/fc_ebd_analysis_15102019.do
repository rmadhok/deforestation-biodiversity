...
program define reg_trip
	...
	** Parse
	...
	eststo: qui reghdfe `depvar' `indepvar' `ctrls', ///
		a(user_id state_code_2011_num) vce(r)
		estadd local user_fe "$\checkmark$"
		...
		estadd local dist_fe ""
	eststo: qui reghdfe `depvar' `indepvar' `ctrls', ///
		a(user_id state_code_2011_num year) vce(r)
		estadd local user_fe "$\checkmark$"
		estadd local state_fe "$\checkmark$"
	...
	eststo: qui reghdfe `depvar' `indepvar' `ctrls', ///
		...
		estadd local dist_fe ""
	eststo: qui reghdfe `depvar' `indepvar' `ctrls', ///
		a(user_id state_code_2011_num#year c_code_2011_num) vce(r)
		estadd local user_fe "$\checkmark$"
		estadd local state_fe ""
		estadd local year_fe ""
		estadd local st_y_fe "$\checkmark$"
...
if `main_analysis' == 1 {
	...
	* Read
	...
	la_var_tex district_forest_cum_ihs district_nonforest_cum_ihs `u_ctrls'
	
/*	
	//1. Species Richness on Deforestation

	reg_trip s_richness_ihs district_forest_cum_ihs district_nonforest_cum_ihs `u_ctrls'
	esttab using "${TABLE}/tables/s_richness_deforest_trip.tex", replace ///
		keep(district* coverage* tree*) stats(user_fe state_fe year_fe st_y_fe ///
		dist_fe N r2, labels(`"User FEs"' `"State FEs"' `"Year FEs"' ///
		`"State $\times$ Year FEs"' `"District FEs"' `"N"' `"\(R^{2}\)"') ///
		fmt(0 0 0 0 0 0 3)) nocons nomtitles star(* .1 ** .05 *** .01) ///
	...
	* Simplify labels
	...
	unab projvars: district_forest_*_cum_ihs
	la_var_tex `projvars'
		
	* Electricity
	reg_trip s_richness_ihs district_forest_elec_cum_ihs district_nonforest_elec_cum_ihs `u_ctrls'
	...
	* Other Projects
	foreach i in ind irr mine o tran fvr pa hyb lin {
		...
		reg_trip s_richness_ihs district_forest_`i'_cum_ihs district_nonforest_`i'_cum_ihs `u_ctrls'
	...
	* Non-linear projects
	reg_trip s_richness_ihs district_forest_nl_cum_ihs district_nonforest_nl_cum_ihs `u_ctrls'
	esttab using "${TABLE}/tables/projwise_s_richness_trip.tex", append f ///
		keep(district_forest_nl_cum_ihs) stats(user_fe state_fe year_fe st_y_fe ///
		dist_fe N r2, labels(`"User FEs"' `"State FEs"' `"Year FEs"' ///
		`"State $\times$ Year FEs"' `"District FEs"' `"N"' `"\(R^{2}\)"') ///
		fmt(0 0 0 0 0 0 3)) nocons nomtitles label nolines nonotes booktabs ///
		star(* .1 ** .05 *** .01) se b(%5.3f) se(%5.3f) width(\hsize) prefoot(\hline)
	eststo clear
*/	
	//3. Species Diversity on Birding Activity
	local u_ba_ctrls district_nonforest_cum_km2 coverage tree_cover_mean temperature_mean precipitation_mean
	la_var_tex district_forest_cum_km2 `u_ba_ctrls'
	...
	* Panel A: Duration
	...
	esttab using "${TABLE}/tables/birding_activity_deforest_trip.tex", replace f ///
		...
		dist_fe N r2, labels(`"User FEs"' `"State FEs"' `"Year FEs"' ///
		`"State $\times$ Year FEs"' `"District FEs"' `"N"' `"\(R^{2}\)"') ///
		fmt(0 0 0 0 0 0 3)) nocons refcat(district_forest_cum_km2 ///
...
if `robustness' == 1 {
	...
	* Read
	...
	la_var_tex district_forest_cum_ihs district_nonforest_cum_ihs `controls'
	
	//1. Species Richness on Deforestation
	reg_dm s_richness_mean_ihs district_forest_cum_ihs district_nonforest_cum_ihs `controls'
	esttab using "${TABLE}/tables/s_richness_mean_deforest.tex", replace ///
		keep(district* coverage* tree*) stats(dist_fe st_y_fe smonth_fe year_fe N r2, ///
	...
	//2. Species Richness on Deforestation by Project Type
	...
	* Electricity
	reg_dm s_richness_mean_ihs district_forest_elec_cum_ihs district_nonforest_elec_cum_ihs `controls'
	esttab using "${TABLE}/tables/s_richness_mean_deforest_projectwise.tex", replace ///
		f keep(district_forest_elec_cum_ihs) mlabels("OLS" "WLS" "OLS" "WLS" "OLS" "WLS") ///
	...
	* Other Projects
	foreach i in ind irr mine o tran fvr pa hyb lin {
		reg_dm s_richness_mean_ihs district_forest_`i'_cum_ihs district_nonforest_`i'_cum_ihs `controls'	
		esttab using "${TABLE}/tables/s_richness_mean_deforest_projectwise.tex", ///
	...
	* Non-Linear
	reg_dm s_richness_mean_ihs district_forest_nl_cum_ihs district_nonforest_nl_cum_ihs `controls'	
	esttab using "${TABLE}/tables/s_richness_mean_deforest_projectwise.tex", ///
		append f keep(district_forest_nl_cum_ihs) stats(dist_fe st_y_fe ///
	...
	local ba_controls district_nonforest_cum_km2 tree_cover_mean coverage_mean temperature_mean precipitation_mean
	la_var_tex district_forest_cum_km2 `ba_controls'

	** Panel A: Duration
	eststo clear
	reg_dm duration_mean district_forest_cum_km2 `ba_controls'
	esttab using "${TABLE}/tables/birding_activity_deforest_mean.tex", replace f ///
		keep(district* coverage* tree*) stats(dist_fe st_y_fe smonth_fe year_fe N r2, ///
		labels(`"District FEs"' `"State $\times$ Year FEs"' `"State $\times$ Month FEs"' ///
		`"Year FE"' `"N"' `"\(R^{2}\)"') fmt(0 0 0 0 0 3)) nocons ///
		mlabels("OLS" "WLS" "OLS" "WLS" "OLS" "WLS") refcat(district_forest_cum_km2 ///
		"\emph{Panel A: Duration (min)}", nolabel) nomtitles ///
		star(* .1 ** .05 *** .01) label nonotes booktabs se b(%5.3f) ///
		se(%5.3f) width(\hsize)
	eststo clear
	...
	* Panel B: Distance
	...
	esttab using "${TABLE}/tables/birding_activity_deforest_mean.tex", append f ///
		keep(district* coverage* tree*) nocons mlabels(none) nomtitles ///
...
if `appendix' == 1 {
	...
	* Panel A: Shannon Index
	reg_trip sh_index_ihs district_forest_cum_ihs district_nonforest_cum_ihs `u_ctrls'
	esttab using "${TABLE}/tables/biodiv_deforest_trip_apx.tex", replace f ///
		keep(district* coverage* tree*) stats(user_fe state_fe year_fe st_y_fe ///
		dist_fe N r2, labels(`"User FEs"' `"State FEs"' `"Year FEs"' ///
		`"State $\times$ Year FEs"' `"District FEs"' `"N"' `"\(R^{2}\)"') ///
		fmt(0 0 0 0 0 0 3)) refcat(district_forest_cum_ihs ///
	...
	* Panel B: Simpson Index
	reg_trip si_index_ihs district_forest_cum_ihs district_nonforest_cum_ihs `u_ctrls'
	esttab using "${TABLE}/tables/biodiv_deforest_trip_apx.tex", append f ///
		keep(district* coverage* tree*) stats(user_fe state_fe year_fe st_y_fe ///
		dist_fe N r2, labels(`"User FEs"' `"State FEs"' `"Year FEs"' ///
		`"State $\times$ Year FEs"' `"District FEs"' `"N"' `"\(R^{2}\)"') ///
		fmt(0 0 0 0 0 0 3)) nocons refcat(district_forest_cum_ihs ///
	...
	foreach y in sh_index si_index {
		...
		* Electricity
		reg_trip `y'_ihs district_forest_elec_cum_ihs ///
			district_nonforest_elec_cum_ihs `u_ctrls'
		...
		* Other Projects
		foreach i in ind irr mine o tran fvr pa hyb lin {
			...
			reg_trip `y'_ihs district_forest_`i'_cum_ihs district_nonforest_`i'_cum_ihs `u_ctrls'
		...
		* Non-linear projects
		reg_trip `y'_ihs district_forest_nl_cum_ihs district_nonforest_nl_cum_ihs `u_ctrls'
		esttab using "${TABLE}/tables/projwise_`y'_trip.tex", append f ///
			keep(district_forest_nl_cum_ihs) stats(user_fe state_fe year_fe st_y_fe ///
		dist_fe N r2, labels(`"User FEs"' `"State FEs"' `"Year FEs"' ///
		`"State $\times$ Year FEs"' `"District FEs"' `"N"' `"\(R^{2}\)"') ///
	...
	* Read
	...
	foreach j in mean md d {
		...
		* Panel A: Species Richness
		reg_dm s_richness_`j'_ihs district_forest_cum_ihs district_nonforest_cum_ihs `controls'
		esttab using "${TABLE}/tables/biodiv_`j'_deforest_apx.tex", replace f ///
			keep(district* coverage* tree*) stats(dist_fe st_y_fe smonth_fe year_fe N r2, ///
			labels(`"District FEs"' `"State $\times$ Year FEs"' `"State $\times$ Month FEs"' ///
			`"Year FE"' `"N"' `"\(R^{2}\)"') fmt(0 0 0 0 0 3)) ///
			mlabels("OLS" "WLS" "OLS" "WLS" "OLS" "WLS") nonotes nonumbers label ///
			booktabs nomtitles nocons refcat(district_forest_cum_ihs ///
			"\emph{Panel A: Species Richness}", nolabel) star(* .1 ** .05 *** .01) ///
			se b(%5.3f) se(%5.3f) width(\hsize)
		eststo clear
		
		* Panel B: Shannon Index
		reg_dm sh_index_`j'_ihs district_forest_cum_ihs district_nonforest_cum_ihs `controls'
		esttab using "${TABLE}/tables/biodiv_`j'_deforest_apx.tex", append f ///
			keep(district* coverage* tree*) stats(dist_fe st_y_fe smonth_fe year_fe N r2, ///
			labels(`"District FEs"' `"State $\times$ Year FEs"' `"State $\times$ Month FEs"' ///
		...
		* Panel C: Simpson Index
		reg_dm si_index_`j'_ihs district_forest_cum_ihs district_nonforest_cum_ihs `controls'
		esttab using "${TABLE}/tables/biodiv_`j'_deforest_apx.tex", append f ///
			keep(district* coverage* tree*) nocons mlabels(none) nomtitles nonumbers ///
	...
	* Panel A: Mean
	foreach var in s_richness_mean sh_index_mean si_index_mean {	
		reg_wls_drop `var'_ihs district_forest_cum_ihs district_nonforest_cum_ihs `controls'
	}
	esttab using "${TABLE}/tables/biodiv_deforest_drop10.tex", replace f ///
		keep(district* coverage* tree*) nocons stats(dist_fe smonth_fe year_fe N r2, ///
		...
		prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
		mlabels("WLS" "Drop" "WLS" "Drop" "WLS" "Drop") refcat(district_forest_cum_ihs ///
	...
	* Panel B: Median
	...
	esttab using "${TABLE}/tables/biodiv_deforest_drop10.tex", append f ///
		keep(district* coverage* tree*) nocons stats(dist_fe smonth_fe year_fe N r2, ///
		labels(`"District FEs"' `"State $\times$ Month FEs"' `"Year FE"' `"N"' ///
	...
	* Panel C: All
	...
	esttab using "${TABLE}/tables/biodiv_deforest_drop10.tex", append f ///
		...
		labels(`"District FEs"' `"State $\times$ Month FEs"' `"Year FE"' `"N"' ///
		`"\(R^{2}\)"') fmt(0 0 0 0 3)) refcat(district_forest_cum_ihs ///
		"\emph{Panel C: All Trips}", nolabel) ///
		star(* .1 ** .05 *** .01) label nonotes mlabels(none) ///
		nomtitles nonumbers booktabs se b(%5.3f) se(%5.3f) width(\hsize)
	eststo clear
	...
	foreach j in mean md d {
		...
		* Project Category
		foreach var in s_richness sh_index si_index {
			...
			esttab using "${TABLE}/tables/biodiv_`j'_deforest_projectwise.tex", replace ///
				f keep(district_forest_elec_cum_ihs) mgroups("Species Richness" ///
				"Shannon Index" "Simpson Index", pattern(1 0 1 0 1 0) ///
		...
		foreach i in ind irr mine o tran fvr pa hyb lin {
			...
			esttab using "${TABLE}/tables/biodiv_`j'_deforest_projectwise.tex", ///
				append f keep(district_forest_`i'_cum_ihs) nolines noobs ///
				label booktabs nomtitles nonotes nocons mlabels(none) ///
				nonumbers refcat(district_forest_hyb_cum_ihs "\emph{Project Shape}", ///
				nolabel)se b(%5.3f) se(%5.3f) star(* .1 ** .05 *** .01)
		...
		foreach var in s_richness sh_index si_index {
			...
			esttab using "${TABLE}/tables/biodiv_`j'_deforest_projectwise.tex", ///
				append f keep(district_forest_nl_cum_ihs) label booktabs ///
	...
	** Coefficient Plot
	...
	local ba_controls district_nonforest_cum_km2 tree_cover_mean coverage temperature_mean precipitation_mean
	la_var_tex district_forest_cum_km2 `ba_controls'

	** Panel A: Duration
	eststo clear
	reg_dm duration_md district_forest_cum_km2 `ba_controls'
	esttab using "${TABLE}/tables/birding_activity_deforest_md.tex", replace f ///
		keep(district* coverage* tree*) stats(dist_fe st_y_fe smonth_fe year_fe N r2, ///
		labels(`"District FEs"' `"State $\times$ Year FEs"' `"State $\times$ Month FEs"' ///
		`"Year FE"' `"N"' `"\(R^{2}\)"') fmt(0 0 0 0 0 3)) nocons ///
		mlabels("OLS" "WLS" "OLS" "WLS" "OLS" "WLS") refcat(district_forest_cum_km2 ///
	...
	* Panel B: Distance
	...
	esttab using "${TABLE}/tables/birding_activity_deforest_md.tex", append f ///
		keep(district* coverage* tree*) nocons mlabels(none) nomtitles ///
		nonumbers stats(dist_fe st_y_fe smonth_fe year_fe N r2, ///
		labels(`"District FEs"' `"State $\times$ Year FEs"' ///
		`"State $\times$ Month FEs"' `"Year FE"' `"N"' `"\(R^{2}\)"') ///
